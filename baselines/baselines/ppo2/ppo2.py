...
class Runner(AbstractEnvRunner):
    ...
    def __init__(self, *, env, model, nsteps, gamma, lam, writer):
        ...
        for _ in range(self.nsteps):
            ...
            for i in range(len(infos)):
                if self.dones[i]:
                    ...
                    if 'forward' in infos[i]['episode']['done']:
                        self.done_forward += 1
                    if 'backward' in infos[i]['episode']['done']:
                        self.done_backward += 1
                    if 'side' in infos[i]['episode']['done']:
                        self.done_side += 1
...
def learn(*, policy, env, nsteps, total_timesteps, ent_coef, lr,
    ...
    if load_path is not None:
        ...
        if osp.exists(ob_rms_path) and osp.exists(ret_rms_path):
            ...
            with open(ret_rms_path, 'rb') as ret_rms_fp:
                env.ret_rms = pickle.load(ret_rms_fp)
            print('normalization checkpoint load...')
    # tensorboard
    timestamp = time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time()))
    writer = tf.summary.FileWriter(osp.join(logger.get_dir(), timestamp), tf.get_default_session().graph)

    runner = Runner(env=env, model=model, nsteps=nsteps, gamma=gamma, lam=lam, writer=writer)
